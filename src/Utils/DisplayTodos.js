...
const DisplayTodos = ({
  ...
  return (
    <>
      {data.length ? (
        <div>
          <div className="max-md:container border text-white max-w-[700px] mt-10 max-sm:mt-5 m-auto rounded-3xl bg-gradient-to-r from-purple-500 to-purple-700 p-10 max-sm:p-6">
            <h1 className=" text-2xl max-sm:text-xl font-medium">
              Progress summery
            </h1>
            <h3>{`${data.length} ${data.length > 1 ? "Tasks" : "Task"}`}</h3>

            <div className="flex flex-col w-[60%] max-sm:w-[100%] mt-7 max-sm:mt-5">
              <div className="flex justify-between items-center">
                <p>
                  Progress
                  <span className={` text-sm ${handleTasksStatus() === "No tasks completed" ? "text-red-700" : handleTasksStatus() === "Less than half tasks completed" ? "text-red-700" : "text-green-500"} font-semibold`}> ({handleTasksStatus()})</span>
                </p>
          ...
          <div className="max-md:container max-w-[700px] m-auto mt-7 max-sm:mt-5 mb-7 max-sm:mb-5 relative">
            <input
              ...
              className="w-full h-14 max-sm:h-12 rounded-xl pl-11 placeholder:text-base outline-none"
            />
            <IoMdSearch className="absolute top-[50%] left-3 -translate-y-[50%] text-purple-600 text-2xl" />
          </div>
